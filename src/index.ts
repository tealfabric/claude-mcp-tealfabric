...
server.registerTool(
  ...
  {
    description:
      "Download a document/file from Tealfabric documents storage. Returns JSON for JSON responses, or a binary-safe base64 payload with filename and content type.",
    inputSchema: z.object({
      file_path: z.string().describe("Full path to the file (e.g. packages/report-v1.zip)"),
      tenant_id: z.string().optional().describe("Tenant ID (defaults to authenticated tenant)"),
    }),
